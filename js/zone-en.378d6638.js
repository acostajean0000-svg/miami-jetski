...
(function(){
  ...
  function injectUX(){
    ...
    search.innerHTML='<span class="ux-search-icon">🔎</span><input type="search" id="uxSearch" placeholder="Search by name, type, location..." autocomplete="off"><button type="button" class="ux-clear" id="uxClear" aria-label="Clear">×</button>';
    var tools=document.createElement('div');
    tools.className='ux-toolbar';
    tools.innerHTML='<div class="ux-result-count" id="uxCount">Loading…</div><div class="ux-sort"><label for="uxSort">Sort:</label><select id="uxSort"><option value="rec">Recommended</option><option value="rating">⭐ Top rated</option><option value="reviews">💬 Most reviewed</option><option value="price-asc">💰 Price: low → high</option><option value="price-desc">💰 Price: high → low</option><option value="name">A–Z</option></select></div>';
  ...
  function renderGrid(list, query){
    ...
    if(!grid)return;
    if(!list.length){
      grid.innerHTML='<div class="ux-empty"><h3>🔍 No matches</h3><p>Try a different search or category.</p>'+(query?'<button class="ux-empty-clear" onclick="document.getElementById(\'uxSearch\').value=\'\';document.getElementById(\'uxSearch\').dispatchEvent(new Event(\'input\'))">Clear search</button>':'')+'</div>';
      return;
    }
    ...
    grid.innerHTML=list.slice(0,200).map(function(op,i){
      ...
      var href='/' + encodeURIComponent(slug || (op.name||'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,80)+'-'+(op.zone||''));
      var priceText=(op.price&&op.price>0)?('from $'+op.price):'Contact for pricing';
...
(function(){
  var INSTALL_KEY='mjbr_install_dismissed';
  var SHOW_AFTER_VISITS=2;
  var deferredPrompt=null;
  
  // Track visits
  var visits=parseInt(localStorage.getItem('mjbr_visits')||'0')+1;
  localStorage.setItem('mjbr_visits',visits);
  
  // Capture install prompt
  window.addEventListener('beforeinstallprompt',function(e){
    e.preventDefault();
    deferredPrompt=e;
    setTimeout(maybeShowBar,visits>=SHOW_AFTER_VISITS?12000:60000);
  });
  ...
  function maybeShowBar(){
    ...
    bar.className='pwa-install-bar show';
    bar.innerHTML='<div class="pwa-install-icon">📲</div><div class="pwa-install-text"><div class="pwa-install-title">Install our app</div><div class="pwa-install-sub">Faster booking · Works offline · Add to home screen</div></div><button class="pwa-install-btn" id="pwaInstallBtn">Install</button><button class="pwa-install-close" id="pwaCloseBtn" aria-label="Close">×</button>';
    document.body.appendChild(bar);
    document.getElementById('pwaInstallBtn').addEventListener('click',function(){
      if(window.gtag)gtag('event','pwa_install_accepted');
      deferredPrompt.prompt();
      deferredPrompt.userChoice.then(function(c){
        if(c.outcome==='accepted'){
          if(window.gtag)gtag('event','pwa_install_completed');
        }
    ...
    document.getElementById('pwaCloseBtn').addEventListener('click',function(){
      localStorage.setItem(INSTALL_KEY,Date.now());
      bar.remove();
      if(window.gtag)gtag('event','pwa_install_dismissed');
    });
    if(window.gtag)gtag('event','pwa_install_prompted',{visits:visits});
  }
  
  // Web Share API integration
  if(navigator.share){
    // Add share button to nav if exists
    var navEl=document.querySelector('nav,.nav');
    if(navEl){
      var btn=document.createElement('button');
      btn.className='pwa-share-btn available';
      btn.innerHTML='📤 Share';
      btn.onclick=function(){
        navigator.share({
          title:document.title,
          text:'Check out 10,000+ verified water sports operators across 37 destinations',
          url:location.href
        }).then(function(){
          if(window.gtag)gtag('event','native_share',{method:'web_share_api'});
        }).catch(function(){});
      };
  ...
  if(window.matchMedia('(display-mode: standalone)').matches){
    if(window.gtag)gtag('event','pwa_session',{installed:true});
  ...
  window.addEventListener('appinstalled',function(){
    ...
    var existing=document.querySelector('.pwa-install-bar');
    if(existing)existing.remove();
  });
  
  // Online/offline events
  window.addEventListener('online',function(){
    if(document.querySelector('.offline-banner'))document.querySelector('.offline-banner').remove();
  });
  window.addEventListener('offline',function(){
    if(document.querySelector('.offline-banner'))return;
    var b=document.createElement('div');
    b.className='offline-banner';
    b.style.cssText='position:fixed;top:0;left:0;right:0;background:#ff6b6b;color:#fff;padding:8px;text-align:center;font-size:.82rem;font-weight:700;z-index:9999';
...
(function(){
  ...
  function injectControls(){
    ...
    ov.innerHTML=
      '<button class="map-control-btn" id="mapRadiusToggle">🎯 Radius</button>'+
      '<button class="map-control-btn" id="mapLayerToggle" data-mode="street">🛰️ Satellite</button>'+
      '<button class="map-control-btn" id="mapFullscreenToggle">⛶ Full</button>';
    wrap.appendChild(ov);
    ...
    rp.innerHTML='<button data-r="0.047348">250 ft</button><button data-r="0.094697">500 ft</button><button data-r="0.189394">1,000 ft</button><button data-r="0.284091">1,500 ft</button><button data-r="0.473485">2,500 ft</button><button data-r="0.757576">4,000 ft</button><button data-r="0">Off</button>';
    wrap.appendChild(rp);
    ...
    vc.innerHTML='<strong>0</strong> operators in view';
    wrap.appendChild(vc);
  ...
  function bindControls(){
    ...
    $('#mapFullscreenToggle').onclick=function(){
      ...
      this.innerHTML=wrap.classList.contains('fullscreen')?'✕ Exit':'⛶ Full';
    ...
    document.addEventListener('keydown',function(e){
      if(e.key==='Escape'&&$('.map-wrap').classList.contains('fullscreen')){
        ...
        $('#mapFullscreenToggle').innerHTML='⛶ Full';
  ...
  function updateVisibleCount(explicit){
    ...
    el.innerHTML='<strong>'+count+'</strong> operator'+(count===1?'':'s')+' in view';
  }
...
(function(){
  ...
  function updateFavBtn(){
    ...
    if(cnt>0){if(cntEl){cntEl.textContent=cnt}else{btn.innerHTML='❤️ Favs <span class="mp-fav-count">'+cnt+'</span>'}}
    else{btn.innerHTML='❤️ Favs'}
  }
  ...
  function addToCompare(opId){
    ...
    if(compareList.length>=3){alert('Maximum 3 operators for comparison');return}
  ...
  function updateCompareBar(){
    ...
    if(!bar){
      ...
      bar.id='mpCompareBar';
      bar.innerHTML='<span class="mp-cmp-count" id="mpCmpCount">0</span><span style="color:#e8f4fd;font-size:.82rem">selected to compare</span><button id="mpCmpShow">Compare →</button><button class="mp-cmp-clear" id="mpCmpClear">Clear</button>';
      document.body.appendChild(bar);
  ...
  function showCompareModal(){
    ...
    if(!modal){
      ...
      modal.id='mpCmpModal';
      modal.innerHTML='<div class="mp-modal"><button class="mp-modal-close" onclick="document.getElementById(\'mpCmpModal\').classList.remove(\'show\')" aria-label="Close">×</button><h3>Compare Operators</h3><div class="mp-cmp-grid" id="mpCmpGrid"></div></div>';
  ...
  var CAT_KEYWORDS={
    ...
    watersports:['water sports','watersport','wakeboard','flyboard','parasail','jet pack','tube','banana boat','paddleboard','sup','kayak'],
  ...
  function renderSearchResults(opResults,placeResults){
    ...
    if(!html){html='<div class="mp-result">No results</div>'}
    box.innerHTML=html;
  ...
  function applyFilters(){
    ...
    if(vc)vc.innerHTML='<strong>'+ops.length+'</strong> operator'+(ops.length===1?'':'s')+' in view';
  ...
  function injectUI(){
    ...
    sb.innerHTML='<span class="mp-search-icon">🔎</span><input type="search" id="mpSearchInput" placeholder="Search jet ski, boat, marina, address..." autocomplete="off"><button id="mpSearchBtn">Go</button>';
    wrap.appendChild(sb);
  ...
  function showFavoritesModal(){
    ...
    if(!modal){
      ...
      modal.className='mp-modal-overlay';
      modal.id='mpFavModal';
      modal.innerHTML='<div class="mp-modal"><button class="mp-modal-close" onclick="document.getElementById(\'mpFavModal\').classList.remove(\'show\')" aria-label="Close">×</button><h3>❤️ My Favorites</h3><div class="mp-cmp-grid" id="mpFavGrid"></div></div>';
      document.body.appendChild(modal);
    }
    var grid=document.getElementById('mpFavGrid');
    if(!ops.length){
      grid.innerHTML='<p style="color:#a8d4f0;grid-column:1/-1;text-align:center;padding:30px">No favorites yet. Click ❤️ on any operator pin to save it here.</p>';
    } else {
      grid.innerHTML=ops.map(function(o){
        var photo=o.photo&&o.photo.includes('http')?o.photo:(o.photo?'https://cdn.filestackcontent.com/'+o.photo+'/convert?w=400&format=webp':'');
        return '<div class="mp-cmp-card"><img src="'+photo+'" alt="'+(o.name||'').replace(/"/g,'&quot;')+'" onerror="this.style.display=\'none\'"><div class="mp-cmp-body"><h4>'+(o.name||'').slice(0,60)+'</h4><div class="mp-cmp-attr">⭐ <strong>'+(o.rating||'4.7')+'</strong> · 💰 $'+(o.price||'50')+'+</div><a href="/'+getSlug(o)+'" class="mp-cmp-book">View Details →</a></div></div>';
      }).join('');
    }
    modal.classList.add('show');
  }
  ...
  function enrichPopup(map){
    map.on('popupopen',function(e){
      ...
      var slug=getSlug(op);
      var __ll=userLoc||(window.mapInstance&&window.mapInstance.getCenter?window.mapInstance.getCenter():null);var __dmi=(__ll&&op.lat&&typeof haversineMi==='function')?haversineMi(__ll.lat,__ll.lng,op.lat,op.lng):null;var distance=(__dmi!=null)?('~'+Math.max(1,Math.round(__dmi*20))+' min walk · '+__dmi.toFixed(1)+' mi'):'';
      var fav=isFav(op.id);
      ...
      var html='<div class="mp-popup">'+
        ...
        ((function(){var t=[];var em=(window.CAT_EMOJI&&window.CAT_EMOJI[op.cat])||'';if(op.badge)t.push((em?em+' ':'')+op.badge);if(op.zl)t.push('📍 '+op.zl);if(op.pax)t.push('👥 up to '+op.pax);return t.length?'<div class="mp-popup-tags">'+t.join(' · ')+'</div>':'';})())+(distance?'<div class="mp-popup-distance">📍 '+distance+'</div>':'')+
        '<div class="mp-popup-actions">'+
        ((op.link&&op.link.indexOf('http')===0)?'<a href="'+op.link+'" target="_blank" rel="noopener" class="mp-popup-btn">⚡ Book Now</a>':'<a href="/'+slug+'" class="mp-popup-btn">View Details</a>')+
        ...
        '</div>'+
        '<a href="/'+slug+'" class="mp-popup-btn secondary" style="width:100%;margin-top:6px">View Details</a>'+
        '<button class="mp-popup-btn secondary" style="width:100%;margin-top:8px" onclick="window.MPcompare(\''+op.id+'\')">+ Add to Compare</button>'+
        '</div></div>';
...
(function(){
  ...
  var CAT_DISPLAY={
    ...
    watersports:{label:'Water Sports',emoji:'🏄'},
    bikerental:{label:'Bike & E-Ride',emoji:'🚲'},
    exotic:{label:'Exotic Cars',emoji:'💎'},
    atv:{label:'ATV',emoji:'🏍️'},
    yacht:{label:'Yacht',emoji:'🛳️'},
    jetski:{label:'Jet Ski',emoji:'🛥️'},
    segway:{label:'Segway',emoji:'🛴'},
    airboat:{label:'Airboat',emoji:'🐊'},
    snorkel:{label:'Snorkel',emoji:'🤿'},
    fishing:{label:'Fishing',emoji:'🎣'},
    villa:{label:'Villa',emoji:'🏡'},
    walking_tour:{label:'Walking Tour',emoji:'🚶'},
    culinary:{label:'Food & Wine',emoji:'🍷'},
    aerial:{label:'Aerial Tour',emoji:'✈️'},
    sunset:{label:'Sunset',emoji:'🌅'},
    slingshot:{label:'Slingshot',emoji:'🏎️'},
    golfcart:{label:'Golf Cart',emoji:'🛺'},
    nightlife:{label:'Nightlife',emoji:'🍸'},
    jetcar:{label:'Jet Car',emoji:'🚗'},
    wildlife:{label:'Wildlife',emoji:'🐋'},
    flowers:{label:'Bloom Bar',emoji:'💐'}
  };
  ...
  function injectCatChips(){
    ...
    html+='<button class="mp-cat-chips-clear" id="mpCatChipsClear">✕ Clear</button>';
  ...
  function applyMapCatFilter(){
    ...
    var vc=document.getElementById('mapVisibleCount')||document.getElementById('hpVisibleCount');
    if(vc){
      var label=selectedCats.size>0?' filtered':' in view';
...
(function(){
  ...
  function injectSyncBtn(){
    ...
    btn.id='mpSyncBtn';
    btn.innerHTML='<span class="mp-sync-icon">🔄</span> Sort by map';
    btn.title='Reorder cards by distance from map center';
    ...
    btn.onclick=function(){
      ...
      if(syncEnabled){
        btn.classList.add('active');
        btn.innerHTML='<span class="mp-sync-icon">✓</span> Sorted by map';
        sortCardsByDistance();
